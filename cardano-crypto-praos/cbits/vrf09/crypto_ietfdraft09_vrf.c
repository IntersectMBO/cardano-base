...
{
    return crypto_vrf_ietfdraft09_PRIMITIVE;
}

int
crypto_vrf_keypair(unsigned char *pk, unsigned char *sk)
{
    return crypto_vrf_ietfdraft09_keypair(pk, sk);
}
...
{
    return crypto_vrf_ietfdraft09_keypair_from_seed(pk, sk, seed);
}

int
crypto_vrf_is_valid_key(const unsigned char *pk)
{
    return crypto_vrf_ietfdraft09_is_valid_key(pk);
}
...
{
    return crypto_vrf_ietfdraft09_prove(proof, skpk, m, mlen);
}
...
{
    return crypto_vrf_ietfdraft09_verify(output, pk, proof, m, mlen);
}

int
crypto_vrf_proof_to_hash(unsigned char *hash, const unsigned char *proof)
{
    return crypto_vrf_ietfdraft09_proof_to_hash(hash, proof);
}

void
crypto_vrf_sk_to_pk(unsigned char *pk, const unsigned char *skpk)
{
    crypto_vrf_ietfdraft09_sk_to_pk(pk, skpk);
}

void
crypto_vrf_sk_to_seed(unsigned char *seed, const unsigned char *skpk)
{
    crypto_vrf_ietfdraft09_sk_to_seed(seed, skpk);
}
