...
instance (HashAlgorithm h, Typeable d) => FixedSizeCodec (VerKeyKES (SumKES h d)) where
...
instance
  ...
  rawDecodeFixedSized b = do
    guardFixedSized (Proxy @(SigKES (SumKES h d))) b
...
instance
  ...
  rawDecodeFixedSized b = do
    guardFixedSized (Proxy @(UnsoundPureSignKeyKES (SumKES h d))) b
