...
instance (KESAlgorithm v, Typeable a) => FixedSizeCodec (SignedKES v a) where
...
encodeSignedKES :: (KESAlgorithm v, Typeable a) => SignedKES v a -> Encoding
encodeSignedKES = encodeFixedSized
{-# DEPRECATED encodeSignedKES "Use `encodeFixedSized` instead" #-}

decodeSignedKES :: (KESAlgorithm v, Typeable a) => Decoder s (SignedKES v a)
