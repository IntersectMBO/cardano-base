...
deriving instance
  (KESAlgorithm d, Eq (SignKeyKES d)) =>
    ...
    [ testGroup
        ...
        [ testGroup
          ...
          testGroup
            "size"
            [ testProperty "VerKey" $
                prop_size_serialise @(VerKeyKES v)
                  rawSerialiseVerKeyKES
                  (sizeVerKeyKES (Proxy @ v)),
              testProperty "SignKey" $
                prop_size_serialise @(SignKeyKES v)
                  rawSerialiseSignKeyKES
                  (sizeSignKeyKES (Proxy @ v)),
              testProperty "Sig" $
                prop_size_serialise @(SigKES v)
                  rawSerialiseSigKES
                  (sizeSigKES (Proxy @ v))
            ],
          testGroup
            "direct CBOR"
            [ testProperty "VerKey" $
                prop_cbor_with @(VerKeyKES v)
                  encodeVerKeyKES
                  decodeVerKeyKES,
              testProperty "SignKey" $
                prop_cbor_with @(SignKeyKES v)
                  encodeSignKeyKES
                  decodeSignKeyKES,
              testProperty "Sig" $
                prop_cbor_with @(SigKES v)
                  encodeSigKES
                  decodeSigKES
            ],
          testGroup
          ...
          testGroup
            ...
            [ testProperty "VerKey" $
                prop_cbor_direct_vs_class @(VerKeyKES v)
                  encodeVerKeyKES,
              testProperty "SignKey" $
                prop_cbor_direct_vs_class @(SignKeyKES v)
                  encodeSignKeyKES,
              testProperty "Sig" $
                prop_cbor_direct_vs_class @(SigKES v)
                  encodeSigKES
            ]
        ],
