...
testBLSCurve name _ =
  testGroup
    ...
    , testProperty "inefficient MSM matches naive approach" $ \(psAndSs :: [(BLS.Point curve, BigInteger)]) ->
        let pairs = [(p, i) | (p, BigInteger i) <- psAndSs]
         in BLS.blsMSM pairs
              === foldr (\(p, s) acc -> BLS.blsAddOrDouble acc (BLS.blsMult p s)) (BLS.blsZero @curve) pairs
    , testProperty "efficient MSM matches naive approach" $ \(psAndSs :: [(BLS.Point curve, BigInteger)]) ->
        let pairs = [(p, i) | (p, BigInteger i) <- psAndSs]
         in BLS.blsMSM' pairs
              === foldr (\(p, s) acc -> BLS.blsAddOrDouble acc (BLS.blsMult p s)) (BLS.blsZero @curve) pairs
